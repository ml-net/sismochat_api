...
- Commit messages: concise subject ONLY (lowercase, no period). No body, no bullet points, no details. Implementation details go as comments on the GitHub issue, NOT in commits. Do NOT put `Closes #N` in commit messages — put it in the PR description.
- CI/CD: GitHub Actions runs tests on Node 18 and 20; `JWT_SECRET` is provided from GitHub Secrets and must never be hardcoded. CHANGELOG: update as the last commit in the feature PR, just before merge. Then tag on main after merge. Every merge to main that triggers a deploy MUST have a corresponding version tag.
...
- Copilot-specific rules: never push directly to `main`; when proposing changes create a branch and PR; do not auto-merge; ensure commit messages follow Conventional Commits (concise subject ONLY, no body, no details); never commit secrets or hard-coded secrets (JWT_SECRET must come from environment/secrets); always create PRs as draft (mark ready only after explicit approval); never amend commits — use new commits (PR history shows all changes, squash merges them); put `Closes #N` in PR description only; CHANGELOG update is the last commit in the feature PR, just before merge.
